...
class Detector:
    ...
    def check_if_human(self, face_img):
        ...
        eyes = self.eye_cascade.detectMultiScale(face_img, 2, 9,minSize=(20,20))
        mouth = self.mouth_cascade.detectMultiScale(face_img, 2, 9,minSize=(20,20))
        nose = self.nose_cascade.detectMultiScale(face_img, 2, 9,minSize=(20,20))
        check_skin=skindetect.check_skin(face_img ,0.1)
        check_face=len(eyes) >1 or  (len(mouth) == 1 and len(nose) == 1)
    ...
    def detect(self,img):
        ...
        faces = self.frontal_face_cascade.detectMultiScale(gray, scaleFactor=1.2,minNeighbors=5, minSize=(40,40),flags=cv2.CASCADE_SCALE_IMAGE)
